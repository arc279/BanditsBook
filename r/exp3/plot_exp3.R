...
caption = sprintf("Exp3 [horizon: %d / simulates: %d]", horizon, num_sims)
...
names(sims) <- c("Gamma", "Sim", "T", "ChosenArm", "Reward", "CumulativeReward")
sims <- transform(sims, Gamma = factor(Gamma))

# Plot average reward as a function of time.
stats1 <- ddply(sims,
               c("Gamma", "T"),
               function (df) {mean(df$Reward)})
a <- ggplot(stats1, aes(x = T, y = V1, group = Gamma, color = Gamma)) +
  geom_line() +
  ylim(0, 1) +
...
b <- ggplot(stats2, aes(x = T, y = V1, group = Gamma, color = Gamma)) +
  geom_line() +
  ylim(0, 1) +
...
c <- ggplot(stats3, aes(x = T, y = V1, group = Gamma, color = Gamma)) +
  geom_line() +
...
d <- ggplot(stats4, aes(x = T, y = V1, group = Gamma, color = Gamma)) +
  geom_line() +
...
names(results) <- c("Gamma", "Arm", "Weight")
results <- transform(results, Gamma = factor(Gamma))

result1 <- ddply(results,
                 c("Gamma", "Arm"),
                 function (df) {mean(df$Weight)})
m <- ggplot(result1, aes(x = Arm, y = V1, group = Gamma, color = Gamma)) +
  geom_line() +
  xlim(0, numOfArms) +
  ylim(0, horizon) +
  labs(x="Arm", y="Count") +
